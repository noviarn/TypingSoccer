...
enum FieldBuilder {
    ...
    @discardableResult
    static func build(in parent: SKNode, sceneSize: CGSize) -> Geometry {
        // Field runs edge-to-edge left/right. Vertically it's a fraction of
        // the space below the HUD — tweak fieldHeightScale to taste.
        let fieldHeightScale: CGFloat = 0.82
        let availableHeight = sceneSize.height - GameConfig.hudHeight
        let fieldHeight = availableHeight * fieldHeightScale
        let bottomMargin = availableHeight - fieldHeight
        ...
        pitch.fillColor = SKColor(
            red: 110/255,
            green: 155/255,
            blue: 107/255,
            alpha: 1
        )
        pitch.strokeColor = SKColor(white: 1, alpha: 0.8)
        pitch.lineWidth = 2
        pitch.zPosition = 0
        ...
        halfway.path = hp
        halfway.strokeColor = SKColor(white: 1, alpha: 0.6)
        ...
        circle.strokeColor = SKColor(white: 1, alpha: 0.6)
        circle.lineWidth = 1.5
        parent.addChild(circle)
        
        // Three lanes.
        let laneYs: [CGFloat] = [
//            rect.minY + rect.height * 0.80,
...
//            rect.minY + rect.height * 0.20,
        ...
        for y in laneYs {
            ...
            lp.move(to: CGPoint(x: rect.minX, y: y))
            lane.path = lp
            lane.strokeColor = SKColor(white: 1, alpha: 0.5)
            lane.lineWidth = 1
            // dashed
...
/// Minimal scene whose only job is to call FieldBuilder so it can be
/// rendered in Xcode's canvas without needing the full GameScene.
private final class FieldPreviewScene: SKScene {
    override func didMove(to view: SKView) {
        backgroundColor = SKColor(red: 0.02, green: 0.03, blue: 0.05, alpha: 1)
        scaleMode = .aspectFit
        FieldBuilder.build(in: self, sceneSize: size)
    }
}

#Preview {
    let size = CGSize(width: 900, height: 560)
    return SpriteView(scene: FieldPreviewScene(size: size))
}
